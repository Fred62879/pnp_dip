...
def train(img_fn, mask_fn, output_dir, ratio, noise_sigma,
          sigma_0, L, prior, sample_intvl, num_iter, shrinkage_param, rho, recon_dir, metric_dir):
#def train(x_true, sigma_0, L, prior, sample_intvl, num_iter, shrinkage_param, rho, A, recon_dir, metric_dir):
...
#          recon_dir, metric_dir):
    ...
    #A, At, A_diag = A_inpainting(mask_fn, ratio, x_true.numel(), dtype)
    A, At, A_diag = A_inpainting(ratio, x_true.numel())
    ...
    for t in range(num_iter):
        ...
        if t == 0 or (t + 1) % sample_intvl == 0:
            ...
            imsave(fn, recon[0].transpose((1, 2, 0)))
...
if __name__ == '__main__':
    ...
    #      scaled_lambda_, shrinkage_param, rho, prox_op,
    #      recon_dir, metric_dir)
    train(img_fn, mask_fn, output_dir, mask_ratio, noise_sigma,
          sigma_0, L, prior, sample_intvl, num_iters, shrinkage_param, rho, recon_dir, metric_dir)
